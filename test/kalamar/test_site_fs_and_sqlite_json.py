...
class TestSite(object):
    ...
    def setUp(self):
        ...
        self.site = Site(jsonparser.parse(os.path.join(self.temp_dir,
                                      'kalamar_fs_and_sqlite.json')),
...
site = Site(jsonparser.parse(os.path.join(os.path.dirname(__file__), 'data',
                         'kalamar_fs_and_sqlite.json')), 
            fail_on_inexistent_parser=False)
...
for access_point in site.access_points:
    for test in (TestSiteSearch, TestSiteOpen, TestSiteSave,
        ...
        cls = type(str(test.__name__+'_'+access_point), (TestSite, test, TestCase),
