...
from .requests import (LiteralRequest, ContextRequest, as_chain,
                       WithRealAttributes)
from .wrappers import (RequestWrapper, EqWrapper, BinaryOperationWrapper,
                       AndWrapper, LiteralWrapper)


def split_predicate(filter, types, contexts=()):
    """Takes a FilterWrapper object,
    and returns two predicates wrappers objects"""
...
def inner_split(wrapped_request, types, contexts):
    named_types = [type for type in wrapped_request.used_types(contexts).keys()
                   if type.name]
    if not set(named_types) - set(types):
        # `expression` already only concern the desired types
        return (wrapped_request, RequestWrapper.from_request(
            LiteralRequest(True)))
    elif isinstance(wrapped_request, AndWrapper):
        subject1, subject2 = inner_split(
            wrapped_request.subject, types, contexts)
        ...
        return (RequestWrapper.from_request(LiteralRequest(True)),
                wrapped_request)
...
def cut_on_predicate(request, predicate, recursive=False, position=-1):
    ...
    When a request_part matches the predicate,
    the chain is cut at the relative position `position`.
    ...
    for idx, request_part in enumerate(chain):
        if matcher(request_part):
            ...
            tail = WithRealAttributes(chain[-1])._copy_replace(
                {chain[idx + position]:
                empty_context})
    ...
    if isinstance(expression, BinaryOperationWrapper):
        if isinstance(expression, EqWrapper):
            ...
            for this in expression.subject, expression.other:
                subject_value, subject_remainder = isolate_values(
                    this.wrapped_request, contexts)
    ...
    def visitor(req, scope_depth):
        ...
        if isinstance(chain[0], ContextRequest):
            ...
            if wracontext.scope_depth <= -scope_depth and req is not chain[0]:
                ...
                newreq = wrareq._copy_replace({chain[0]: LiteralRequest(
                    context_values[wracontext.scope_depth])})
...
def isolate_identity_values(filter, id_types, contexts=()):
    ...
    self_filter, other_filter = inner_split(
