...
ISO8601_REGEX_DATE = re.compile(r"(?P<year>[0-9]{4})(-(?P<month>[0-9]{1,2})(-(?P<day>[0-9]{1,2})))")
ISO8601_REGEX_DATETIME = re.compile(r"(?P<year>[0-9]{4})(-(?P<month>[0-9]{1,2})(-(?P<day>[0-9]{1,2})"
...
def parse_datetime(datestring, default_timezone=UTC):
    ...
    m = ISO8601_REGEX_DATETIME.match(datestring)
    if not m:
...
def parse_date(datestring, default_timezone=UTC):
    ...
    if not isinstance(datestring, basestring):
        raise ParseError("Expecting a string %r" % datestring)
    m = ISO8601_REGEX_DATE.match(datestring)
    if not m:
        raise ParseError("Unable to parse date string %r" % datestring)
    groups = m.groupdict()
